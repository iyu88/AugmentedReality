...
test_points.geometry.setAttribute(
  ...
  new THREE.BufferAttribute(
    new Float32Array(threeMpPose.numSrcLandmarks() * 3),
    3
  )
);
scene.add(test_points);
const test_new_points = new THREE.Points(
  new THREE.BufferGeometry(),
  new THREE.PointsMaterial({
    color: 0x0000ff,
    size: 0.1,
...
test_new_points.geometry.setAttribute(
  "position",
  new THREE.BufferAttribute(new Float32Array(3 * 8), 3)
);
...
function onResults2(results) {
  ...
  if (skeleton != null) {
    ...
    model.position.copy(
      new THREE.Vector3().addVectors(pos_hip_mp, new THREE.Vector3(1, 0, -1))
    );
...
function createPanel() {
  ...
  panel.add(settings, "show skeleton").onChange(function (visibility) {
    skeleton_helper.visible = visibility;
  });
  //panel.add( settings, 'show local axes' ).onChange( function(visibility){} );
  let gui_text = panel
    .add(settings, "show bone axes")
    .onFinishChange(function (boneName) {
