...
class ThreeMpPose {
  ...
  add3dJointsForMixamo = function () {
    const center_hips = new Vector3().addVectors(
      this.pose3dDict["left_hip"],
      this.pose3dDict["right_hip"]
    );
    ...
    const center_shoulders = new Vector3().addVectors(
      mp_left_shoulder,
      mp_right_shoulder
    );
    center_shoulders.multiplyScalar(0.5);

    const center_ear = new Vector3().addVectors(
      this.pose3dDict["left_ear"],
      this.pose3dDict["right_ear"]
    );
  ...
  transformToWorld = function (camera, dist_from_cam, offset) {
    ...
    for (const [key, value] of Object.entries(this.poseLandmarks)) {
      ...
      p_3d = camera.isPerspectiveCamera
        ? ProjScale(p_3d, camera.position, camera.near, dist_from_cam)
  ...
  rigSolverForMixamo = function (skeleton) {
    ...
    this.computeJointParentR = function (
      nameSkeletonJoint,
      nameMpJoint,
      nameMpJointParent,
      R_chain,
      ...
      const v = new Vector3()
        .subVectors(
          this.pose3dDict[nameMpJoint],
          this.pose3dDict[nameMpJointParent]
        )
        .normalize();
    ...
    let R_chain_spines;
    {
      ...
      skeleton
        .getBoneByName("mixamorigSpine")
        .quaternion.slerp(
          new Quaternion().setFromRotationMatrix(R_spine0),
          0.9
        );

      R_chain.multiply(R_spine0);
      const R_spine1 = this.computeJointParentR(
        "mixamorigSpine2",
        "spine2",
        "spine1",
        R_chain,
      ...
      const R_spine2 = this.computeJointParentR(
        "mixamorigNeck",
        "neck",
        "spine2",
        R_chain,
    ...
    // neck and head
    {
      ...
      const R_neck = this.computeJointParentR(
        "mixamorigHead",
        "head",
        "neck",
        R_chain,
      ...
      const R_headR = this.computeJointParentR(
        "mixamorigRightEye",
        "right_eye",
        "head",
        R_chain,
    ...
    {
      let R_chain = R_chain_spines.clone();
      const R_shoulder_left = this.computeJointParentR(
        "mixamorigLeftArm",
        "left_shoulder",
        "shoulder_left",
        R_chain,
      ...
      const R_arm = this.computeJointParentR(
      ...
      const R_forearm = this.computeJointParentR(
        "mixamorigLeftHand",
        "left_wrist",
        "left_elbow",
        R_chain,
      ...
      const R_hand = this.computeJointParentR(
    ...
    {
      let R_chain = R_chain_spines.clone();
      const R_shoulder_right = this.computeJointParentR(
        "mixamorigRightArm",
        "right_shoulder",
        "shoulder_right",
        R_chain,
      ...
      skeleton
        .getBoneByName("mixamorigRightArm")
        .quaternion.slerp(new Quaternion().setFromRotationMatrix(R_arm), 0.9);
    ...
    {
      let R_chain = R_hips.clone();
      const R_upleg = this.computeJointParentR(
        "mixamorigLeftLeg",
        "left_knee",
        "left_hip",
        R_chain,
      ...
      const R_leg = this.computeJointParentR(
        "mixamorigLeftFoot",
        "left_ankle",
        "left_knee",
        R_chain,
      ...
      skeleton
        .getBoneByName("mixamorigLeftFoot")
    ...
    // right legs
    {
      ...
      skeleton
        .getBoneByName("mixamorigRightLeg")
        .quaternion.slerp(new Quaternion().setFromRotationMatrix(R_leg), 0.9);
  ...
  computeR = function (A, B) {
    ...
    const R = new Matrix4().multiplyMatrices(
      C.clone().transpose(),
  ...
  static decompose_R_ZYX = function (R) {
    ...
    const thetaY = Math.atan2(
      -elements[2],
      new Vector2(elements[6], elements[10]).length()
    );
